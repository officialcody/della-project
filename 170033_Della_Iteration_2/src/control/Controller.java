 ...
 * </p>
 *
 * @author Harry Sameshima, Modified by Sparsh Goel
 * @version 1
 */
public class Controller {
	...
	/*
	 * This constructor is private and synchronized because we are using the
	 * singleton design pattern and we want only one!
	 */
	private Controller() {
	}
	...
	public static synchronized Controller getInstance() {
		// If theController is not initialized, we start by trying to read the intial
		// state from the xml file
		if (theController == null) {
			theController = (Controller) DataManager.readData();

			// If theController is still null, we were not able to initialize from the xml
			// file
			if (theController == null) {
	...
	/**
	 * Save the current controller and all of the data objects rooted here to the
	 * persistent store
	 */
	public void save() {
		dirtyFlag = false;
	...
	public void setActionItemManager(ActionItemManager x) {
		actionItemManager = x;
	}

	public void setDirtyFlag(boolean x) {
		dirtyFlag = x;
	}
	...
	public void setElementlist(Elementlist x) {
		ei = x;
	}
